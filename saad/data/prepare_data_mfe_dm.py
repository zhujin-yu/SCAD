...
def image_convert_bytes(img):
    buffer = BytesIO()
    img.save(buffer, format='tif')
    return buffer.getvalue()


def resize_multiple(img, sizes=(64, 256), resample=Image.BICUBIC, lmdb_save=False):
...
def prepare_process_worker(wctx, file_subset):
    for file in file_subset:
        ...
        if not wctx.lmdb_save:
            lr_img.save(
                '{}/lr_{}/{}.tif'.format(wctx.out_path, wctx.sizes[0], i.zfill(5)))
            hr_img.save(
                '{}/hr_{}/{}.tif'.format(wctx.out_path, wctx.sizes[1], i.zfill(5)))
            sr_img.save(
                '{}/sr_{}_{}/{}.tif'.format(wctx.out_path, wctx.sizes[0], wctx.sizes[1], i.zfill(5)))
        else:
...
def prepare(img_path, out_path, n_worker, sizes=(64, 256), resample=Image.BICUBIC, lmdb_save=False):
    ...
    if n_worker > 1:
        ...
        for file in tqdm(files):
            ...
            if not lmdb_save:
                lr_img.save(
                    '{}/lr_{}/{}.tif'.format(out_path, sizes[0], i.zfill(5)))
                hr_img.save(
                    '{}/hr_{}/{}.tif'.format(out_path, sizes[1], i.zfill(5)))
                sr_img.save(
                    '{}/sr_{}_{}/{}.tif'.format(out_path, sizes[0], sizes[1], i.zfill(5)))
            else:
